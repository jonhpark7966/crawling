...
while True:
    ...
    for lawyer in lawyers:
        ...
        if len(company_sid) == 2:
            ...
            # write first row
            f_writer.writerow([id_generator(), words[0], words[1], company, name, birth, sex, bar_exam, bar_period, lawyer_exam, major, staple, company, job, position, st_years[0], ed_years[0], highs[0], univs[0], grads[0], careers[0], address])
            # write rows
            for i in range(1, len(grads)):
                f_writer.writerow(['', '','', '', '', '', '', '', '', '', '', '', '', '', '', st_years[i], ed_years[i], highs[i], univs[i], grads[i], careers[i], ''])

        elif len(company_sid) == 1:#NO LAWYER
            f_writer.writerow(['', words[0], words[1], company_sid[0].strip(), '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', '', ''])
